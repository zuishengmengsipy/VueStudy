...
Vue.js 2.0 版本推荐使用 axios 来完成 ajax 请求，而不是自己开发的Vue-resource。参考资料

{% embed url="https://www.runoob.com/vue2/vuejs-ajax-axios.html" %}

Axios 是一个基于 Promise 的 HTTP 库，可以用在浏览器和 node.js 中。Github开源地址： [https://github.com/axios/axios](https://github.com/axios/axios)
...
<script>
  import axios from "axios" //这里需要导入
  export default {
    name: "axios_view",
    data() {
      return {
...
**注意，这里每个页面都要导入import axios from “axios”如果想要直接使用this.$axios而不是import后axios，则需要在main.js里的Vue原型链中添加$axios**
...
**这样，在子组件中，就可以不导入直接使用this.$axios了，mounted\(\)可改写成如下**
...
### **跨域的解决方案（出现**has been blocked by CORS policy: The 'Access-Control-Allow-Origin'这种）

{% embed url="https://www.cnblogs.com/GGGG-XXXX/article/9893381.html" %}

{% embed url="https://blog.csdn.net/u011072037/article/details/90694032" %}
...
axios\(config\) axios\({ method: 'post', url: '/user/12345', data: { firstName: 'Fred', lastName: 'Flintstone' } }\); axios\({ method:'get', url:'http://bit.ly/2mTM3nY', responseType:'stream' }\) .then\(function\(response\) { response.data.pipe\(fs.createWriteStream\('ada\_lovelace.jpg'\)\) }\); axios\(url\[, config\]\) axios\('/user/12345'\);
